...
public:
    ...
    *  (int tol) of the (RGBAPixel average) color stored in the root of the subtree. 
...
private:
	...
	void renderHelper(Node* root, PNG &pngee);

	void pruneHelper(Node* root, double pct, int tol);
		
	void clearHelper(Node* root);
	
	twoDtree::Node * copyHelper(Node* orig);

	void countLeaves(Node* curr, Node* subRoot, int tol, double & numLeaves,double & goodLeaves);
